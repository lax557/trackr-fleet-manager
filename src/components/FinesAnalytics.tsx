...
import { format, subMonths, isWithinInterval, differenceInDays, isBefore } from 'date-fns';
...
const STATUS_COLORS = [
  'hsl(221.2 83.2% 53.3%)',   // OPEN - blue
  'hsl(45 93% 47%)',          // DUE_SOON - amber
  'hsl(0 72.2% 50.6%)',       // OVERDUE - red
  'hsl(142.1 76.2% 36.3%)',   // PAID - green
  'hsl(262.1 83.3% 57.8%)',   // CONTESTED - purple
  'hsl(220 8.9% 46.1%)',      // CANCELED - gray
];

const SEVERITY_COLORS = [
  'hsl(142.1 76.2% 36.3%)',   // LEVE - green
  'hsl(45 93% 47%)',          // MEDIA - yellow
  'hsl(24.6 95% 53.1%)',      // GRAVE - orange
  'hsl(0 72.2% 50.6%)',       // GRAVISSIMA - red
];
...
export function FinesAnalytics() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (vehicleFilter) {
      data = data.filter(f => f.vehicleId === vehicleFilter);
    }
  ...
  const monthlyData = useMemo(() => {
    ...
    filteredData.forEach(f => {
      ...
      if (!months[monthKey]) {
        months[monthKey] = { month: monthLabel, count: 0, amount: 0 };
      }
  ...
  const statusDistribution = useMemo(() => {
    ...
    return Object.entries(statuses)
      ...
      .map(([key, value]) => ({
        name: fineStatusLabels[key as FineStatusType],
        value,
      }));
  }, [filteredData]);
  ...
  const severityDistribution = useMemo(() => {
    ...
    return Object.entries(severities).map(([key, val]) => ({
      name: fineSeverityLabels[key as FineSeverity],
      count: val.count,
      amount: val.amount,
    }));
  }, [filteredData]);
  ...
  const topVehicles = useMemo(() => {
    ...
    filteredData.forEach(f => {
      if (!vehicles[f.vehicleId]) {
        vehicles[f.vehicleId] = { plate: f.vehiclePlate || f.vehicleId, count: 0, amount: 0 };
      }
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <Card>
          <CardContent className="pt-4">
            ...
            <p className="text-2xl font-bold mt-1">
              R$ {kpis.totalWithDiscount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </p>
          </CardContent>
        </Card>
        <Card>
              ...
              <BarChart data={severityDistribution} layout="vertical">
                ...
                <ChartTooltip 
                  formatter={(value: number, name: string, props: { payload?: { amount?: number } }) => [
                    `${value} multas (R$ ${(props.payload?.amount ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })})`,
                    name
                  ]}
                />
